--- source http://blog.sqlauthority.com/2011/07/13/sql-server-query-to-find-duplicate-indexes-script-to-find-redundant-indexes/
...
INNER JOIN sys.objects Obj ON Idx.[object_id] = Obj.[object_id] INNER JOIN sys.schemas Sch ON Sch.[schema_id] = Obj.[schema_id]
INNER JOIN sys.dm_db_partition_stats AS s ON s.[object_id] = idx.[object_id] AND s.[index_id] = idx.[index_id]
WHERE idx.index_id > 0
GROUP BY Sch.[name],Obj.[name],Idx.[name],Idx.index_id)

SELECT    MD1.SchemaName, MD1.TableName, MD1.IndexName,MD1.IndexSizeMB,
