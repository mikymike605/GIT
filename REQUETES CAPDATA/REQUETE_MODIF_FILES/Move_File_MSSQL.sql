...
DECLARE @DataPath nvarchar(255) = 'N:\SAPDATA5\'


DECLARE @LogPath nvarchar(255) = 'C:\Share_SQL\SQL_LOG\'
...
Select b.name as DBName, a.name as BdLogicalFileName, a.type_desc as DBFileType, RIGHT(physical_name,CHARINDEX('\',physical_name)) as Extension, reverse(left(reverse(physical_name), charindex('\', reverse(physical_name)) -1)) as PhysicalFileName 
--,physical_name
from master.sys.master_files a inner join sys.databases b
