...
SELECT   'ALTER INDEX ['+[dbindexes].[name]+  '] on [' +SCHEMA_NAME(dbschemas.[schema_id])+'].['+dbtables.[name]+'] REBUILD WITH (ONLINE=ON);'
,SCHEMA_NAME(dbschemas.[schema_id]),
indexstats.avg_fragmentation_in_percent
--FROM sys.dm_db_index_physical_stats (21, 37575172, NULL, NULL,NULL) AS indexstats
FROM sys.dm_db_index_physical_stats (Db_ID('MDW'), NULL, NULL, NULL, 'LIMITED') AS indexstats
INNER JOIN sys.tables dbtables on dbtables.[object_id] = indexstats.[object_id]
INNER JOIN sys.schemas dbschemas on dbtables.[schema_id] = dbschemas.[schema_id]
INNER JOIN sys.indexes AS dbindexes ON dbindexes.[object_id] = indexstats.[object_id]
AND indexstats.index_id = dbindexes.index_id
WHERE indexstats.database_id = DB_ID()
--and avg_fragmentation_in_percent >=05
and avg_fragmentation_in_percent >=30
and dbindexes.[name] is not null
--and dbtables.name='SEM_TICKET'
ORDER BY indexstats.avg_fragmentation_in_percent desc


--ALTER INDEX IDX_ParentInvoice on MDInvoiceDetail REBUILD WITH (ONLINE=ON)	98,3858278309629
--ALTER INDEX PK_MDPayment on MDPayment REBUILD WITH (ONLINE=ON)	46,4084339821426
...
SELECT 'EXEC AdminSQL..maj_statistiques_base ''' + name + ''', ''OUI'' ; print ''*******''+''' + name + '''+''*****'''
FROM sys.databases
