...
where o.name ='F_PRODUCT'  --TICKET_UNIFIE, SEM_TICKET, MD5_INVOICE_DETAIL, F_PRODUCT_TIMESLOT,F_PRODUCT,MD5_INVOICE,MD5_PAYEMENT
and rows >0
--and i.index_id >0
order by 2,3
--SELECT * FROM #tmp
...
SELECT 'ALTER INDEX ['+[indexes].[name]+  '] on ['+DB_NAME(@DatabaseID)+'].[' +SCHEMA_NAME(schemas.[schema_id])+'].['+objects.[name]+'] REBUILD PARTITION ='+cast(dm_db_index_physical_stats.partition_number as varchar)+' WITH (SORT_IN_TEMPDB = OFF, ONLINE = ON);',
...
--FROM sys.dm_db_index_physical_stats (db_id(), NULL, NULL, NULL, 'LIMITED') dm_db_index_physical_stats ------1/12/14/15
FROM sys.dm_db_index_physical_stats (db_id('SID_PRD'), OBJECT_ID('DWH.F_PRODUCT'),1,null,'limited') dm_db_index_physical_stats
...
INNER JOIN sys.dm_db_partition_stats s ON s.[object_id] = indexes.[object_id]
	AND s.[index_id] = indexes.[index_id]
...
--and dbtables.name=''
group by indexes.name,schemas.schema_id
